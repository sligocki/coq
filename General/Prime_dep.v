(* Like Prime.v but with dependent typing. *)
...
Require Import Specif Program.

Program Fixpoint eq_nat_dep (x y : nat) : {x = y} + {x <> y} :=
  match x, y with
    | O, O => left _
    | S x', S y' =>
      match eq_nat_dep x' y' with
        | left _ => left _
        | right _ => right _
      end
    | _, _ => right _
...
Next Obligation.
  destruct x; destruct y; try omega. exfalso. apply (H x y). split; reflexivity.
Defined.
Next Obligation.
  split; intros; intros contra; destruct contra. inversion H. inversion H0.
Defined.
Next Obligation.
  split; intros; intros contra; destruct contra. inversion H0. inversion H.
Defined.

Definition eq_nat_dep' (x y : nat) : {x = y} + {x <> y}.
  decide equality.
Defined.

Fixpoint divide_helper' n m k
  : n <> 0 -> {k'1 | m = n * k'1} + {forall k'2, k'2 <= k -> m <> n * k'2}.
  refine (
    match eq_nat_dep m (n * k) with
    | left _ => fun _ => inleft _
    | right _ =>
      match k with
        | O => fun _ => inright _
        | S k'3 => fun Hn0 =>
          match divide_helper' n m k'3 Hn0 with
            | inright _ => inright _
            | inleft _ => inleft _
          end
      end
  end).
  - eapply exist. apply e.
  - intros. apply le_n_0_eq in H. subst. (* k = 0 *) admit.
  - apply s.
  - intros. remember (S k'3) as k'4. destruct H.
    + (* k = S k'3 *) admit.
    + inversion Heqk'4. subst. apply n1. apply H.


Program Fixpoint divide_helper (n : nat | n <> 0) (m : nat | m <> 0) k
  : {k'1 | m = n * k'1} + {forall k'2, k'2 <= k -> m <> n * k'2} :=
  match eq_nat_dep (n * k) m with
    | left _ => inleft k
    | right _ =>
      match k with
        | O => inright _
        | S k'3 =>
          match divide_helper n m k'3 with
            | inright k'4 => inright k'4
            | inleft no_k'4 => inleft no_k'4
          end
      end
  end.

Next Obligation. Preterm. Admitted.
Next Obligation.
...
Proof.
  intros. apply divmod_divides in H0.
  - destruct H0 as [d3 Hmult]. destruct (divides_reflect p n); [left; assumption|].
    destruct (divides_reflect p m); [right; assumption|]. destruct H.
    apply divmod_not_divides2 in H0; [|omega]. destruct H0 as [dn [mn [Hmn0 Hdivn]]].
    apply divmod_not_divides2 in H1; [|omega]. destruct H1 as [dm [mm [Hmm0 Hdivm]]].
    destruct (divmod_complete p (mn * mm)) as [d3' [m3 Hdiv3]]. omega.
    apply (divmod_mult p n m mn mm m3 dn dm d3') in Hdivn; try assumption.
    destruct Hdivn. apply (divmod_unique p _ d3 x 0 m3) in Hmult.
    + destruct Hmult. subst. rewrite divmod_equiv in Hdiv3. destruct Hdiv3.
      rewrite plus_0_r in H1. admit. (* TODO *)
    + omega.
    + assumption.
  - apply prime_gt_1 in H. omega.
Abort.

Theorem prime_factor_unique: forall n ps,
  n > 1 -> n = prod ps -> (forall p, In p ps -> prime p) -> InOrder ps ->
  prime_factor n ps.
Proof.
  intros n ps. generalize dependent n. induction ps as [| p1 [| p2 ps]]; intros.
  - simpl in H0. rewrite H0 in H. omega.
  - simpl in H0. rewrite mult_1_r in H0. subst. apply pfact_prime.
    apply smallest_factor_prime. apply H1. simpl. left. reflexivity.
  - remember (prod (p2 :: ps)) as m.
    assert (Hp1: prime p1). { apply H1. left. reflexivity. }
    assert (Hp2: prime p2). { apply H1. right. left. reflexivity. }
    apply pfact_comp with (m:=m).
    + remember (smallest_factor n) as fact. symmetry in Heqfact.
      destruct fact as [[p n'] |].
      * assert (Hmult: p * n' = n). { apply smallest_factor_mult. assumption. }
        { apply smallest_factor_smallest with (k:=p1) in Heqfact.
        - (* p <= p1 *) apply le_lt_or_eq in Heqfact. destruct Heqfact.
          + (* p < p1 *) (* TODO: If divides p (prod ps) -> In p ps *) admit.
          + (* p = p1 *) assert (Hmult2: p1 * m = n). { rewrite Heqm. simpl.
               simpl in H0. symmetry. assumption. } rewrite <- Hmult2 in Hmult.
            subst p. apply mult_reg_l in Hmult.
            * rewrite Hmult. reflexivity.
            * destruct Hp1. omega.
        - omega.
        - rewrite H0. apply divides_prod. simpl. left. reflexivity.
        - apply prime_gt_1. assumption. }
      * (* ~ prime (prod ps) *) apply smallest_factor_prime2 in Heqfact; trivial.
        destruct Heqfact. exfalso. unfold not in H4. { apply (H4 p1).
        - apply prime_gt_1. assumption.
        - (* p1 < n *) rewrite H0. simpl. assert (Hgt: 1 < prod (p2 :: ps)). {
            apply prod_prime_gt_1. intros. apply H1. right. assumption. }
          apply mult_lt_compat_l with (p:=p1) in Hgt. 
          + rewrite mult_1_r in Hgt. assumption.
          + destruct Hp1. omega.
        - (* divides p1 n *) exists m. rewrite H0. rewrite Heqm. reflexivity. }
    + apply IHps.
      * rewrite Heqm. { apply le_trans with (m:=p2).
        - destruct Hp2. assumption.
        - apply le_prod.
          + intros. apply lt_le_weak. apply prime_gt_1. apply H1. right. assumption.
