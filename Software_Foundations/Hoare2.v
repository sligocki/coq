...
Proof.
  intros. apply hoare_consequence_pre with (fun st => parity (st X) = parity m).
  eapply hoare_consequence_post. apply hoare_while. eapply hoare_consequence_pre.
  apply hoare_asgn.
  - (* parity X = parity m /\ 2 <= X ->> parity (X - 2) = parity m *)
    intros st. intros. unfold assn_sub. simpl. rewrite t_update_eq. destruct H.
    rewrite <- H. apply parity_ge_2. unfold bassn,beval in H0. apply leb_iff in H0.
    simpl in H0. assumption.
  - (* parity X = parity m /\ ~ 2 <= X ->> X = parity m *) simpl. intros st. intros.
    destruct H. rewrite <- H. symmetry. apply parity_lt_2.
    unfold bassn,beval in H0. intros contra. apply H0. apply leb_iff. simpl.
    assumption.
  - (* X = m ->> parity X = parity m *) intros st. intros. rewrite H. reflexivity.
Qed.
(** [] *)
...
Fixpoint sum_pow n :=
  ...
  Z ::= 1;;
    {{ Y = sum_pow X /\ Z = 2^X }}
  WHILE X <> m DO
      {{ Y = sum_pow X /\ Z = 2^X /\ X <> m }} ->>
      {{ Y + Z = sum_pow (X + 1) /\ 2 * Z = 2^(X + 1) }}
    Z ::= 2 * Z;;
      {{ Y + Z = sum_pow (X + 1) /\ Z = 2^(X + 1) }}
    Y ::= Y + Z;;
      {{ Y = sum_pow (X + 1) /\ Z = 2^(X + 1) }}
    X ::= X + 1
      {{ Y = sum_pow X /\ Z = 2^X }}
  END
    {{ Y = sum_pow X /\ Z = 2^X /\ X = m }} ->>
...
Proof.
  split.
  - (* {{P}} c {{Q}} *) eapply hoare_consequence_pre. apply hoare_asgn.
    intros st. intros. unfold assn_sub. simpl. rewrite t_update_eq. omega.
  - (* Weakest *) intros. intros st. intros. eapply (H st) in H0. Focus 2.
    + apply E_Ass. simpl. reflexivity.
    + rewrite t_update_eq in H0. omega.
Qed.
(** [] *)
...
Theorem hoare_asgn_weakest : forall Q X a,
  is_wp (Q [X |-> a]) (X ::= a) Q.
Proof.
  intros. split. apply hoare_asgn. (* {{Q [X |-> a]}} X ::= a {{Q}} *)
  intros. intros st. intros. unfold assn_sub. eapply (H st) in H0. Focus 2.
...
Lemma hoare_havoc_weakest : forall (P Q : Assertion) (X : id),
  ...
  P ->> havoc_pre X Q.
Proof.
  intros. intros st. intros. unfold havoc_pre. intros.
  apply (H st (t_update st X n)) in H0. assumption. apply E_Havoc.
Qed.
End Himp2.
(** [] *)
...
Lemma fact_const : forall n, n <> 0 -> real_fact n = n * real_fact (n - 1).
Proof.
  intros. destruct n. omega. (* n = 0 *)
  (* n = S n *) simpl. rewrite <- minus_n_O. reflexivity.
Qed.
...
Definition factorial_dec (m : nat) : dcom := (
    {{ fun st => st X = m }} ->>
...
Proof.
  intros. verify.
  - (* X * (X-1)! = X! *) rewrite <- mult_assoc. rewrite <- fact_const.
    assumption. assumption.
  - (* 0! = 1 *) simpl in H. omega.
Qed.
(** [] *)
