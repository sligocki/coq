...
Function countoddmembers (l:natlist) : nat :=
  length (oddmembers l).
...
Fixpoint alternate (l1 l2 : natlist) : natlist :=
  ...
  | x :: rest =>
    match l2 with
    | nil => l1
    | y :: rest2 => x :: y :: alternate rest rest2
    end
  end.
...
(* TODO: Is it possible as a Function? *)
Function member (v:nat) (s:bag) : bool :=
...
Fixpoint subset (s1:bag) (s2:bag) : bool :=
  match s1 with
  | nil => true
  | x :: rest => andb (negb (beq_nat (count x s2) O))
                      (subset rest (remove_one x s2))
  end.
...
Lemma rev_anti_comm : forall l k : natlist,
  rev (l ++ k) = rev k ++ rev l.
Proof.
  intros.
  induction l.
...
Proof.
  induction l.
  - reflexivity.
  - simpl. rewrite -> rev_anti_comm. rewrite -> IHl. reflexivity.
Qed.
...
Fixpoint beq_natlist (l1 l2 : natlist) : bool :=
  ...
  | cons x1 rest1 =>
    match l2 with
    | nil => false
    | cons x2 rest2 => andb (beq_nat x1 x2) (beq_natlist rest1 rest2)
    end
  end.
...
Theorem count_and_sum: forall (s t : bag) (n : nat),
  count n (sum s t) = (count n s) + (count n t).
Proof.
  intros.
  induction s.
  - reflexivity.
...
Proof. destruct x. simpl. apply beq_nat_refl.
Qed.
(** [] *)
...
Zero. Both constructors Baz1 and Baz2 require an already existing
object of type baz to construct a new one. There's no place to start.
*)
(** [] *)
