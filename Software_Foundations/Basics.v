...
Proof. reflexivity. Qed. (** TODO: simpl doesn't seem to do anything... **)
...
Fixpoint incr (n : bin) : bin :=
  match n with
    ...
    | twice k => twice_plus_one k
    | twice_plus_one k => twice (incr k)
  end.

Fixpoint bin_to_nat (n : bin) : nat :=
  match n with
    | zero => O
    | twice k => plus (bin_to_nat k) (bin_to_nat k)
    | twice_plus_one k => S (plus (bin_to_nat k) (bin_to_nat k))
  end.
...
Example test_bin0:
  (bin_to_nat (twice zero)) = (bin_to_nat zero).
Proof. reflexivity. Qed.

Example test_bin_incr0: (bin_to_nat zero) = 0.
Proof. reflexivity. Qed.
