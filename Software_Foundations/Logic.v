...
Theorem contrapositive : forall P Q : Prop,
  (P -> Q) -> (~Q -> ~P).
Proof.
  intros. intros HP. apply H0. apply H. apply HP.
Qed.
(** [] *)
...
Theorem not_both_true_and_false : forall P : Prop,
  ~ (P /\ ~P).
Proof.
  unfold not. intros P [HP HnP]. apply HnP. apply HP.
Qed.
(** [] *)
...
Proof.
  intros. intros contra. destruct contra. elim H0. apply H.
Qed.
(** [] *)
...
Proof.
  ...
  - induction l.
    + intros. elim H0.
    + intros H_All x0 H_In. simpl in H_In. simpl in H_All. destruct H_All.
      destruct H_In.
      * rewrite <- H1. apply H.
      * { apply IHl.
        - apply H0.
...
Proof.
  induction l1.
  - reflexivity.
  - simpl. intros. rewrite -> IHl1. rewrite -> (IHl1 [x]). rewrite <- app_assoc. simpl.
    reflexivity.
Qed.
...
Proof.
  ...
  - reflexivity.
  - simpl. rewrite <- IHl. unfold tr_rev. simpl. apply rev_append_app.
Qed.
(** [] *)
...
Proof.
  ...
  - destruct IHn as [k IHn]. rewrite evenb_S. destruct (evenb n).
    + simpl. exists k. rewrite <- IHn. reflexivity.
    + simpl. exists (S k). rewrite -> IHn. rewrite -> double_plus.
      rewrite -> double_plus. simpl. rewrite <- plus_n_Sm. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros. split.
  - intros. intros contra. apply beq_nat_true_iff in contra. rewrite H in contra.
    inversion contra.
  - intros. destruct (beq_nat x y) eqn:H1.
    + apply beq_nat_true_iff in H1. apply H in H1. inversion H1.
    + reflexivity.
Qed.
(** [] *)
...
Fixpoint beq_list {A} (beq : A -> A -> bool)
  ...
  | x1 :: rest1 => match l2 with
    | [] => false
    | x2 :: rest2 => (beq x1 x2) && beq_list beq rest1 rest2
    end
  end.
...
Proof.
  ...
  - split.
    + intros. destruct l2.
      * inversion H0.
      * inversion H0. apply andb_true_iff in H2. destruct H2. apply IHl1 in H2.
        apply H in H1. rewrite H1. rewrite H2. reflexivity.
...
Proof.
  ...
  - intros. induction l.
    + reflexivity.
    + simpl. apply andb_true_iff. simpl in H. destruct H. split.
      * apply H.
      * apply IHl. apply H0.
Qed.
...
Theorem forall_exists: forall X (P : X -> Prop) (x : X),
  P x -> exists x, P x.
Proof. intros. exists x. apply H. Qed.
...
Proof.
  intros. unfold excluded_middle in H. destruct (H (P x)).
  - apply H1.
  - assert (H2: exists x, ~ P x). { exists x. apply H1. }
...
Theorem excluded_middle1: excluded_middle <-> peirce.
Proof.
  unfold excluded_middle. unfold peirce. split.
  - intros. destruct (H P).
    + apply H1.
    + apply H0. intro. apply H1 in H2. inversion H2.
  - intros. (* apply H. *) Abort.

Theorem excluded_middle2: excluded_middle <-> double_negation_elimination.
Proof.
...
Theorem excluded_middle3: excluded_middle <-> de_morgan_not_and_not.
  unfold excluded_middle. unfold de_morgan_not_and_not. split.
  - intros. destruct (H P).
    + left. apply H1.
    + destruct (H Q).
      * right. apply H2.
      * assert (H3: ~P /\ ~Q). { split. apply H1. apply H2. } contradiction.
  - intros. apply H. apply not_both_true_and_false.
Qed.

Theorem excluded_middle4: excluded_middle <-> implies_to_or.
  unfold excluded_middle. unfold implies_to_or. split.
  - intros. destruct (H P).
    + right. apply H0. apply H1.
    + left. apply H1.
  - intros. apply or_comm. apply H. intros. apply H0.
Qed.
(** [] *)
