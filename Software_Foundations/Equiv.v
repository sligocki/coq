...
(* while x > 0:
     x++
   Halts iff x == 0. No change to state. *)
...
(* Halts for all states. No change to state. *)
Definition prog_c : com :=
  SKIP.

(* while x != 0:
     x = x*y + 1
   Halts iff x == 0. No change to state. = prog_a *)
Definition prog_d : com :=
  WHILE BNot (BEq (AId X) (ANum 0)) DO
    X ::= APlus (AMult (AId X) (AId Y)) (ANum 1)
  END.

(* Always halts. Sets y = 0. = prog_b *)
...
(* while x != y:
     x = y + 1
   Halts iff x == y. No change to state. *)
...
Proof.
  ...
  - inversion H; subst.
    ...
    + apply E_IfTrue.
      ...
      * assumption.
  - inversion H; subst.
    + inversion H5; subst. destruct (beval st b) eqn:Heval. inversion H1.
      apply E_IfFalse.
...
Proof.
  ...
  - apply WHILE_true_nonterm with b c st st' in H. contradiction.
  - exfalso. apply WHILE_true_nonterm with BTrue SKIP st st'.
...
Proof.
  intros. intros st st'. split; intros.
  - inversion H; subst. inversion H2; subst. rename st'0 into st'2.
    apply E_Seq with st'1. assumption. apply E_Seq with st'2.
...
Proof.
  ...
  assert (loop / empty_state \\ empty_state). { apply contra. apply E_Skip. }
...
Inductive ceval : com -> state -> state -> Prop :=
  ...
  | E_Havoc : forall (st : state) (n : nat) (X : id),
      (HAVOC X) / st \\ t_update st X n  (* Havoc could update X with any n. *)
...
Proof.
  left. intros st st'. split; intros.
  - inversion H; subst. inversion H2; subst. inversion H5; subst.
    rewrite t_update_permute.
    + apply E_Seq with (t_update st Y n0). apply E_Havoc. apply E_Havoc.
    + (* X <> Y *) intros contra. inversion contra.
  - admit. (* Should be same proof. *)
Qed.
(** [] *)
...
Proof.
  ...
  assert (ptwice / empty_state \\ t_update (t_update empty_state X 0) Y 1). {
    apply E_Seq with (t_update empty_state X 0). apply E_Havoc. apply E_Havoc. }
  apply contra in H. inversion H; subst. inversion H5; subst. inversion H2; subst.
  simpl in H6. rewrite t_update_eq in H6.
  assert (t_update (t_update empty_state X n) Y n Y =
          t_update (t_update empty_state X 0) Y 1 Y). { rewrite H6. reflexivity. }
  rewrite t_update_eq in H0. rewrite t_update_eq in H0. subst.
  assert (t_update (t_update empty_state X 1) Y 1 X =
          t_update (t_update empty_state X 0) Y 1 X). { rewrite H6. reflexivity. }
  rewrite t_update_neq in H0. rewrite t_update_eq in H0.
  rewrite t_update_neq in H0. rewrite t_update_eq in H0.
  - inversion H0.
  - (* Y <> X *) intros Heq. inversion Heq.
  - (* Y <> X *) intros Heq. inversion Heq.
Qed.
(** [] *)
...
Lemma p1_may_diverge : forall st st', st X <> 0 ->
  ~ p1 / st \\ st'.
Proof. 
  intros. intros contra. remember p1 as while. induction contra; inversion Heqwhile; subst.
  - (* While End *) simpl in H0. apply beq_nat_false_iff in H.
    rewrite H in H0. inversion H0.
  - (* While Loop *) apply IHcontra2; try assumption. inversion contra1; subst.
    inversion H6; subst. rewrite t_update_eq. simpl. omega.
Qed.

Lemma p2_may_diverge : forall st st', st X <> 0 ->
  ~ p2 / st \\ st'.
Proof.
  intros. intros contra. remember p2 as while. induction contra; inversion Heqwhile; subst.
  - (* While End *) simpl in H0. apply beq_nat_false_iff in H.
    rewrite H in H0. inversion H0.
  - (* While Loop *) apply IHcontra2; try assumption. inversion contra1. subst. assumption.
Qed.
...
Proof.
 intros st st'. destruct (NPeano.Nat.eqb_spec (st X) 0).
 - (* st X = 0 *) unfold p1,p2. split; intros.
   + inversion H; subst.
     * (* While End *) apply E_WhileEnd. assumption.
     * (* While Loop *) simpl in H2. rewrite e in H2. simpl in H2. inversion H2.
   + inversion H; subst.
     * (* While End *) apply E_WhileEnd. assumption.
     * (* While Loop *) simpl in H2. rewrite e in H2. simpl in H2. inversion H2.
 - (* st X <> 0 *) split; intros.
   + apply p1_may_diverge in H. contradiction. assumption.
   + apply p2_may_diverge in H. contradiction. assumption.
Qed.
(** [] *)
...
Theorem while_exit : forall b c st st',
  WHILE b DO c END / st \\ st' -> beval st' b = false.
Proof.
  intros. remember (WHILE b DO c END) as while. induction H; inversion Heqwhile; subst.
  - (* While End *) assumption.
  - (* While Loop *) apply IHceval2. reflexivity.
Qed.

Theorem p3_p4_inequiv : ~ cequiv p3 p4.
Proof.
  unfold cequiv. intros contra.
  assert (p3 / t_update empty_state X 1 \\ t_update (t_update empty_state X 0) Z 0). {
    replace (t_update (t_update empty_state X 0) Z 0) with
    ...
    - apply E_Seq with (t_update (t_update empty_state X 1) Z 1). apply E_Ass. reflexivity.
      ...
      reflexivity. apply E_Seq with (t_update (t_update (t_update empty_state X 1) Z 1) X 0).
      apply E_Havoc. apply E_Havoc. apply E_WhileEnd. reflexivity.
    - rewrite (t_update_permute _ 0 1 X Z). rewrite t_update_shadow. rewrite t_update_shadow.
      reflexivity. intros Heq. inversion Heq. }
  apply contra in H. inversion H; subst. inversion H5; subst. simpl in H6.
  assert (t_update st' Z 1 Z = 1). { apply t_update_eq. } rewrite H6 in H0.
...
Proof.
  ...
  - remember p5 as while. induction H; inversion Heqwhile; subst.
    + (* While End *) clear Heqwhile. simpl in H. apply negb_false_iff in H.
      apply beq_nat_true_iff in H. assert (p6 / st \\ t_update st X 1). {
        apply E_Ass. simpl. reflexivity. }
      rewrite <- H in H0. rewrite t_update_same in H0. assumption.
    + (* While Loop *) apply IHceval2 in Heqwhile. clear IHceval1 H IHceval2.
      rename Heqwhile into IH. inversion H0; subst. inversion IH; subst.
      simpl in *. rewrite t_update_shadow. apply E_Ass. reflexivity.
  - inversion H; subst. simpl in *. destruct (beq_nat (st X) 1) eqn:Heq.
    + (* X = 1 *) apply beq_nat_true_iff in Heq. rewrite <- Heq. rewrite t_update_same.
      apply E_WhileEnd. simpl. rewrite Heq. reflexivity.
    + (* X <> 1 *) apply E_WhileLoop with (t_update st X 1). simpl. rewrite Heq.
      reflexivity. apply E_Havoc. apply E_WhileEnd. simpl. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - inversion H2; subst. inversion H5; subst. inversion H8; subst.
    ...
    apply E_Seq with (t_update st l2 (aeval st a2)). apply E_Ass. reflexivity.
    replace (aeval (t_update st l1 (aeval st a1)) a2) with (aeval st a2).
    replace (aeval st a1) with (aeval (t_update st l2 (aeval st a2)) a1).
    + apply E_Ass. reflexivity.
...
Definition c4 : com := X ::= ANum 1.

Theorem c3_c4_different : ~ capprox c3 c4 /\ ~ capprox c4 c3.
Proof.
  assert (t_update empty_state X 1 <> empty_state). { intros Heq.
    assert (empty_state X = 0). { reflexivity. } rewrite <- Heq in H.
    rewrite t_update_eq in H. inversion H. }
  split; intros; intros contra.
  - assert (c3 / empty_state \\ empty_state). { apply E_Skip. }
    apply contra in H0. inversion H0. simpl in *. subst. contradiction.
  - assert (c4 / empty_state \\ t_update empty_state X 1). {
      apply E_Ass. reflexivity. } apply contra in H0. inversion H0; subst.
...
Proof.
  intros. intros st st'. intros. apply loop_never_stops in H. contradiction.
Qed.

(** Finally, find a non-trivial property which is preserved by
    program approximation (when going from left to right). *)

Definition zprop (c : com) : Prop := exists st st', c / st \\ st'.


Theorem zprop_preserving : forall c c',
  zprop c -> capprox c c' -> zprop c'.
Proof.
  intros. destruct H as [st [st' H]]. apply H0 in H.
  exists st. exists st'. assumption.
Qed.
(** [] *)
