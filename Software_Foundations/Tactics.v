...
rewrite takes Hypothesis of form FOO = BAR and replaces FOO with BAR
(or vice versa).
apply takes Hypothesis of form forall BLAH, FOO -> BAR. Pattern matches
BAR with current goal and then replaces goal with FOO.
*)
(** [] *)
...
Proof.
  ...
  - intros. simpl in H. destruct n.
    + inversion H.
    + inversion H. rewrite -> H1. apply IHl1 in H1. simpl. rewrite -> H1. reflexivity.
Qed.
(** [] *)
...
(** Now we have plenty to work with: both sides of the equality are
  ...
  rewrite mult_assoc.
  rewrite <- mult_assoc with n m n.
  rewrite mult_comm with m n.
  rewrite mult_assoc.
  rewrite mult_assoc with (n * n) m m.
  reflexivity.
Qed.
...
Proof.
  ...
  - intros. inversion H. destruct xy. destruct (split l) as [l1' l2']. inversion H1.
    assert (H4: combine l1' l2' = l). { apply IHl. reflexivity. }
    unfold combine. unfold combine in H4. rewrite -> H4. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - destruct (f true) eqn:Ht.
    + rewrite -> Ht. rewrite -> Ht. reflexivity. (* f^k true = true *)
    + destruct (f false) eqn:Hf.
      * apply Ht. (* f: true, false => false, true *)
      * apply Hf. (* f: true, false => false, false *)
  - destruct (f false) eqn:Hf.
    + destruct (f true) eqn:Ht.
      * apply Ht.
      * apply Hf.
...
Theorem beq_nat_trans : forall n m p,
  ...
  beq_nat n p = true.
Proof.
  intros. apply beq_nat_eq in H. apply beq_nat_eq in H0.
...
Proof.
  ...
  - intros. simpl. destruct l1.
    ...
    + destruct l2.
      * inversion H.
      * inversion H0.
  - intros. destruct xy. simpl. destruct l1.
    + inversion H0.
    + destruct l2.
      * inversion H0.
      * simpl in H. inversion H. simpl in H0. inversion H0. rewrite -> H5.
        { apply IHl in H2.
...
Proof.
  induction l.
  - intros. inversion H.
  - intros. simpl in H. destruct (test x0) eqn:H_test.
    + inversion H. rewrite <- H1. apply H_test.
...
Fixpoint forallb {X} (f: X -> bool) (l : list X) :=
  match l with
  | [] => true
  | x :: rest => (f x) && (forallb f rest)
  end.

Fixpoint existsb {X} (f: X -> bool) (l : list X) :=
  match l with
  | [] => false
  | x :: rest => (f x) || (existsb f rest)
  end.

Function existsb' {X} (f: X -> bool) (l : list X) :=
  negb (forallb (fun x => negb (f x)) l).

Theorem existsb_existsb': forall X (f : X -> bool) l, existsb f l = existsb' f l.
Proof.
  induction l.
  - reflexivity.
  - simpl. unfold existsb'. simpl.
    assert (H: forall p q, negb (negb p && q) = p || negb q). {
      intros. destruct p.
      + simpl. reflexivity.
      + simpl. reflexivity. }
