...
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun n => leb 8 n) (filter evenb l).
...
Function xorb (x y : bool) : bool := (x || y) && (negb (x && y)).
Notation "x ^ y" := (xorb x y) : bool_scope.

Function checksum (l : list nat) : bool :=
  fold (fun x b => xorb (evenb x) (negb b)) l true.

Example fold_example_diff1 :
  checksum [2; 4] = true.
reflexivity. Qed.

Example fold_example_diff2 :
  checksum [2; 4; 1] = false.
reflexivity. Qed.

Example fold_example_diff3 :
  checksum [2; 4; 1; 3] = true.
reflexivity. Qed.
