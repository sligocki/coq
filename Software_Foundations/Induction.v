...
For numbers, [destruct] allows branching into each case: [O] and [S n']
but this may not be enough to be able to prove something for arbitrary [n].
[induction] allows building a unbounded number of levels of such destructs
so that a proof for 0, 1, 2, ... can be extended to arbitrary [n].
*)
(** [] *)
...
Lemma mult_rev_def: forall m n : nat,
  m * (S n) = m + (m * n).
Proof.
  intros.
  induction m.
...
Proof.
  ...
  - simpl. rewrite <- IHn. rewrite -> mult_rev_def. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  rewrite -> mult_comm with (m:=m).
  induction p.
  - reflexivity.
  - simpl. rewrite -> IHp.
    rewrite -> plus_assoc.
    rewrite -> plus_assoc.
    rewrite <- plus_assoc with (n:=n) (m:=m) (p:=(p * n)).
    rewrite <- plus_assoc with (n:=n) (m:=(p*n)) (p:=m).
...
Proof.
  ...
  - simpl. rewrite <- IHb. simpl.
    rewrite -> plus_comm with (m:=S (bin_to_nat b)). simpl. reflexivity.
Qed.
(** [] *)
...
Proof.
  induction b.
  - reflexivity.
  - simpl. Abort. (* Not true for b = zero *)

Example bin_to_nat_to_bin_twice_zero:
  nat_to_bin (bin_to_nat (twice zero)) = zero.  (* Not = (twice zero) *)
reflexivity. Qed.
(** [] *)
