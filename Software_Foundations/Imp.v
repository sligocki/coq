...
Proof.
  apply E_Seq with (st':=(t_update empty_state X 0)); try (apply E_Ass; reflexivity).
...
Proof.
  ...
  - rewrite H1 in *. rewrite H2 in *. clear H1 H2 b c. clear IHcontra1. apply IHcontra2.
    reflexivity.
Qed.
(** [] *)
...
Inductive no_whilesR: com -> Prop :=
  ...
  | NWAss : forall id a, no_whilesR (CAss id a)
  | NWSeq : forall c1 c2, no_whilesR c1 -> no_whilesR c2 -> no_whilesR (CSeq c1 c2)
  | NWIf : forall b c1 c2, no_whilesR c1 -> no_whilesR c2 -> no_whilesR (CIf b c1 c2).
...
Proof.
  ...
  - induction c; intros; try constructor.
    + simpl in H. apply andb_true_iff in H. destruct H. apply IHc1. apply H.
    + simpl in H. apply andb_true_iff in H. destruct H. apply IHc2. apply H0.
    + simpl in H. apply andb_true_iff in H. destruct H. apply IHc1. apply H.
    + simpl in H. apply andb_true_iff in H. destruct H. apply IHc2. apply H0.
...
Theorem no_whiles_halts : forall c,
  no_whilesR c -> forall st, exists st', c / st \\ st'.
Proof.
  intros c H. induction H; simpl; intros.
  - exists st. constructor.
  - exists (t_update st id (aeval st a)). constructor. reflexivity.
  - destruct (IHno_whilesR1 st) as [st' H1]. destruct (IHno_whilesR2 st') as [st'' H2].
    exists st''. apply E_Seq with (st':=st'). trivial. trivial.
  - destruct (beval st b) eqn:Hb.
    + destruct (IHno_whilesR1 st) as [st' H1]. exists st'. apply E_IfTrue; trivial.
    + destruct (IHno_whilesR2 st) as [st' H1]. exists st'. apply E_IfFalse; trivial.
Qed.
(** [] *)
...
(* If prog succeeds on empty stack, it won't touch anything on non-emtpy stack. *)
Lemma s_exe_irrel : forall st prog stack_in stack_extra stack_out,
  s_execute st stack_in prog = Some stack_out
  -> s_execute st (stack_in ++ stack_extra) prog = Some (stack_out ++ stack_extra).
Proof.
  induction prog; simpl; intros.
  - inversion H. reflexivity.
  - induction a; simpl; try reflexivity.
    + apply IHprog with (stack_extra:=stack_extra) in H. simpl in H. apply H.
    + apply IHprog with (stack_extra:=stack_extra) in H. simpl in H. apply H.
    + destruct stack_in as [| a [| b rest]]; try inversion H. simpl.
      apply IHprog with (stack_extra:=stack_extra) in H. simpl in H. apply H.
    + destruct stack_in as [| a [| b rest]]; try inversion H. simpl.
      apply IHprog with (stack_extra:=stack_extra) in H. simpl in H. apply H.
    + destruct stack_in as [| a [| b rest]]; try inversion H. simpl.
...
Lemma s_exe_app : forall st prog1 prog2 stack stack' stack'',
  s_execute st stack  prog1 = Some stack' ->
  s_execute st stack' prog2 = Some stack'' ->
  s_execute st stack (prog1 ++ prog2) = Some stack''.
Proof.
  induction prog1; intros.
  - inversion H. apply H0.
  - induction a; simpl.
    + apply IHprog1 with (stack':=stack'). simpl in H. apply H. apply H0.
    + apply IHprog1 with (stack':=stack'). simpl in H. apply H. apply H0.
    + destruct stack as [| a [| b rest]]; try inversion H.
      apply IHprog1 with (stack':=stack'). simpl in H. apply H. apply H0.
    + destruct stack as [| a [| b rest]]; try inversion H.
      apply IHprog1 with (stack':=stack'). simpl in H. apply H. apply H0.
    + destruct stack as [| a [| b rest]]; try inversion H.
...
Proof.
  ...
  - apply s_exe_app with (stack':=[aeval st e1]); try trivial.
    apply s_exe_app with (stack':=[aeval st e2; aeval st e1]); try trivial.
    apply s_exe_irrel with (stack_extra:=[aeval st e1]) in IHe2. simpl. apply IHe2.
  - apply s_exe_app with (stack':=[aeval st e1]); try trivial.
    apply s_exe_app with (stack':=[aeval st e2; aeval st e1]); try trivial.
    apply s_exe_irrel with (stack_extra:=[aeval st e1]) in IHe2. simpl. apply IHe2.
  - apply s_exe_app with (stack':=[aeval st e1]); try trivial.
...
Inductive ceval : com -> state -> status -> state -> Prop :=
  ...
  | E_Ass : forall (st : state) (a1 : aexp) (n : nat) (x : id),
      aeval st a1 = n -> (x ::= a1) / st \\ SContinue / t_update st x n
  | E_SeqBreak : forall (c1 c2 : com) (st st' st'' : state),
      c1 / st \\ SBreak / st' -> (c1;; c2) / st \\ SBreak / st'
  | E_SeqContinue : forall (c1 c2 : com) s (st st' st'' : state),
      c1 / st \\ SContinue / st' -> c2 / st' \\ s / st''
      -> (c1;; c2) / st \\ s / st''
  | E_IfTrue : forall s (st st' : state) (b : bexp) (c1 c2 : com),
      beval st b = true ->
      c1 / st \\ s / st' -> (IFB b THEN c1 ELSE c2 FI) / st \\ s / st'
  | E_IfFalse : forall s (st st' : state) (b : bexp) (c1 c2 : com),
      beval st b = false ->
  ...
  | E_WhileBreak : forall (st st' st'' : state) (b : bexp) (c : com),
      beval st b = true ->
      c / st \\ SBreak / st' ->
      (WHILE b DO c END) / st \\ SContinue / st'
  | E_WhileLoop : forall s (st st' st'' : state) (b : bexp) (c : com),
      beval st b = true ->
      c / st \\ SContinue / st' ->
...
Theorem while_stops_on_break : forall b c st st',
  ...
  (WHILE b DO c END) / st \\ SContinue / st'.
Proof.
  intros. apply E_WhileBreak; trivial.
Qed.
...
Proof.
  intros. remember (WHILE b DO c END) as while eqn:Hwhiledef.
  induction H; try inversion Hwhiledef.
  - (* E_WhileFalse *) rewrite H2 in H. rewrite H0 in H. inversion H.
  - (* E_WhileBreak *) rewrite H4 in H1. exists st. trivial.
  - (* E_WhileLoop *)  apply IHceval2; trivial.
Qed.
...
Proof.
  ...
  induction H; intros; inversion H0; subst; try (split; reflexivity).
  (* FILL IN HERE *) Admitted.
