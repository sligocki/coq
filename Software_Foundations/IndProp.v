...
Proof.
  ...
  - intros. induction H.
    ...
    + replace (S (S n)) with (2 + n). apply ev'_sum.
      apply ev'_2. apply IHev. reflexivity.
Qed.
(** [] *)
...
Theorem le_Sm : forall m n, S m <= n -> m <= n.
Proof.
  intros. induction H.
...
Proof.
  intros. induction H.
  - apply H0.
  - apply IHle. apply le_Sm. apply H0.
Qed.
...
Proof.
  intros. apply leb_correct. apply leb_complete in H.
  apply leb_complete in H0. apply (le_trans n m o). apply H. apply H0.
Qed.
...
Inductive subseq {X} : list X -> list X -> Prop :=
  | subseq_eq : forall l, subseq l l
  | subseq_empty : forall l, subseq [] l
  | subseq_comb : forall a1 a2 b1 b2,
      subseq a1 b1 -> subseq a2 b2 -> subseq (a1 ++ a2) (b1 ++ b2).

Theorem subseq_app : forall X (l1 l2 l3: list X),
  subseq l1 l2 -> subseq l1 (l2 ++ l3).
Proof.
  intros. replace (l1) with (l1 ++ []). apply subseq_comb.
  - apply H.
  - apply subseq_empty.
  - apply app_nil_r.
Qed.
...
Proof.
  ...
  - generalize dependent s1. induction s2.
    ...
    + simpl. intros. inversion H. apply IHs2 in H4. rewrite H4. inversion H3.
      simpl. reflexivity. 
  - generalize dependent s1. induction s2.
    + intros. rewrite H. simpl. apply MEmpty.
    + intros. rewrite H. simpl. apply (MApp [x] _ _ _).
      * apply MChar.
...
Proof.
  ...
  - intros. induction re.
    ...
    + simpl in H. apply andb_true_iff in H. destruct H. apply IHre1 in H.
      apply IHre2 in H0. destruct H. destruct H0. exists (x ++ x0). apply MApp.
      * apply H.
      * apply H0.
...
Proof.
  ...
  - inversion Heqre'. apply IHexp_match2 in Heqre'. destruct Heqre'.
    destruct H1. rewrite H2 in IHexp_match1. Abort.
(** [] *)
...
Inductive pal {X} : list X -> Prop :=
  ...
  | pal_expand : forall x sub, pal sub -> pal ([x] ++ sub ++ [x]).
...
Theorem pal_describe : forall X (l : list X),
  pal l <-> ((exists pre, l = pre ++ rev pre)
             \/ (exists pre mid, l = pre ++ [mid] ++ rev pre)).
Proof.
  split.
  - intros. induction H.
    + left. exists []. reflexivity.
    + right. exists []. exists x. simpl. reflexivity.
    + destruct IHpal.
      * destruct H0. left. exists ([x] ++ x0). rewrite rev_app_distr.
        rewrite H0. rewrite <- app_assoc. rewrite <- app_assoc. reflexivity.
      * destruct H0. destruct H0 as [mid H0]. right. exists ([x] ++ x0).
        exists mid. rewrite rev_app_distr. rewrite H0.
        rewrite <- app_assoc. rewrite <- app_assoc. rewrite <- app_assoc.
        reflexivity.
  - intros. destruct H.
    + destruct H. rewrite H. apply pal_app_rev.
    + destruct H as [pre H]. destruct H as [mid H]. rewrite H. 
      generalize dependent l. induction pre.
      * intros. simpl. apply pal_single.
      * intros. simpl. { replace (x :: pre ++ mid :: rev pre ++ [x]) with
                       ([x] ++ (pre ++ [mid] ++ rev pre) ++ [x]).
        - apply pal_expand. apply (IHpre (pre ++ [mid] ++ rev pre)). reflexivity.
        - simpl. rewrite <- app_assoc. simpl. reflexivity. }
Qed.

Theorem rev_pal : forall X (l : list X), l = rev l -> pal l.
Proof.
  intros. apply pal_describe. generalize dependent H. Abort.
(** [] *)
...
Inductive merge {X} : list X -> list X -> list X -> Prop :=
  | merge_empty : merge [] [] []
  | merge_addL : forall x l r t, merge l r t -> merge (x :: l) r (x :: t)
  | merge_addR : forall x l r t, merge l r t -> merge l (x :: r) (x :: t).

Theorem filter_merge : forall X (test : X -> bool) (l l1 l2 : list X),
  merge l1 l2 l ->
...
Proof.
(*
  intros X test l l1. generalize dependent l.
  induction l1.
  - intros l l2 H. replace (l2) with (l).
    + intros. admit.
    + generalize dependent l. induction l2.
      * intros. inversion H. reflexivity.
      * intros. inversion H. apply IHl2 in H4. rewrite H4. reflexivity.
  - intros. 
*)
(*
  intros. generalize dependent test. induction H.
  - simpl. reflexivity.
  - simpl. intros. destruct (test x).
    + intros. replace (filter test t) with (l). reflexivity. symmetry. apply IHmerge.
...
Definition disjoint {X} (l1 l2 : list X) := ~(exists x, In x l1 /\ In x l2).
...
Inductive NoDup {X} : list X -> Prop :=
  | no_dup_empty : NoDup []
  | no_dup_cons : forall x l, NoDup l -> ~(In x l) -> NoDup (x :: l).

(** Finally, state and prove one or more interesting theorems relating
    [disjoint], [NoDup] and [++] (list append).  *)

Theorem disjoin_app : forall X (l1 l2 : list X),
  NoDup l1 -> NoDup l2 -> disjoint l1 l2 -> NoDup (l1 ++ l2).
Proof.
  intros. induction l1.
  - simpl. apply H0.
  - simpl. apply no_dup_cons.
    + apply IHl1.
      * inversion H. apply H4.
      * intros contra. destruct contra. destruct H2. apply H1. exists x0. { split.
        - simpl. right. apply H2.
        - apply H3. }
    + inversion H. intro contra. assert (contra0: In x l1 \/ In x l2). {
        admit. } destruct contra0.
      * contradiction.
...
Proof.
  ...
  - simpl. intros. destruct H.
    ...
    + apply IHl in H. destruct H. destruct H. exists (x0::x1). exists x2.
...
Inductive repeats {X:Type} : list X -> Prop :=
  ...
  | repeats_extend : forall x l, repeats l -> repeats (x :: l).
...
Theorem pigeonhole_principle: forall (X:Type) (l1  l2:list X),
   ...
   repeats l1.
Proof.
   unfold excluded_middle. intros X l1. induction l1 as [|x l1' IHl1'].
   - intros. simpl in H1. unfold lt in H1. inversion H1.
   - intros l2 excluded_middle. intros. destruct (excluded_middle (In x l1')).
     + apply repeats_base. apply H1.
     + apply repeats_extend. apply (IHl1' l2).
  (* FILL IN HERE *) Admitted.
(** [] *)
