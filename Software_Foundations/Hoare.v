...
c adds 5 to X
   ...
   3) {{X <= Y}} c {{Y <= X}}
if X <= Y, c reverses the inequallity
...
c never halts
   ...
   5) {{X = m}}
      c
      {{Y = real_fact m}}
c computes real_fact of X and stores the result in Y

   6) {{True}}
      c
      {{(Z * Z) <= m /\ ~ (((S Z) * (S Z)) <= m)}}
c always leaves Z as largest integer st. Z^2 <= m
*)
...
Theorem hoare_asgn_wrong:
  ~ forall a, {{ fun st => True }} X ::= a {{ fun st => st X = (aeval st a) }}.
Proof.
  intros contra. remember (contra (APlus (AId X) (ANum 1))) as H. simpl in H.
  remember (H empty_state (t_update empty_state X 1)) as H0.
  assert ((fun st : state => st X = st X + 1) (t_update empty_state X 1)). {
    apply H0. apply E_Ass. reflexivity. apply I. } simpl in H1.
...
Proof.
  intros m a P. intros st st'. intros. destruct H0.
  inversion H; subst. rewrite t_update_shadow. rewrite t_update_same. split.
  assumption. rewrite t_update_eq. reflexivity.
Qed.
(** [] *)
...
Theorem hoare_asgn_fwd_exists :
  ...
  {{fun st => exists m, P (t_update st X m) /\
                st X = aeval (t_update st X m) a }}.
Proof.
  intros a P. intros st st'. intros. exists (st X).
  remember hoare_asgn_fwd as HAF. unfold hoare_triple in HAF. apply HAF with st.
...
Proof.
  eapply hoare_consequence_pre. apply hoare_asgn. unfold assert_implies,assn_sub.
...
Proof.
  ...
  - eapply hoare_consequence_pre. apply hoare_asgn. unfold assert_implies,assn_sub.
    intros. simpl. split.
    + rewrite t_update_permute. rewrite t_update_eq. reflexivity. intros Heq. inversion Heq.
...
Proof.
  ...
  - (* Y ::= Z *) apply hoare_asgn.
  - (* X ::= Y *) apply hoare_asgn.
  - (* Z ::= X *) eapply hoare_consequence_pre. apply hoare_asgn.
    unfold assert_implies,assn_sub. intros. simpl.
    repeat (repeat rewrite t_update_eq; rewrite t_update_neq).
    assumption.
...
Proof.
  ...
  - (* Then *)
    ...
    simpl. intros st [_ H]. apply leb_iff in H.
...
Inductive ceval : com -> state -> state -> Prop :=
  ...
  | E_If2False : forall (st : state) (b : bexp) (c : com),
...
Theorem hoare_if1 : forall P Q b c,
  ...
  {{ P }} IF1 b THEN c FI {{ Q }}.
Proof.
  intros. intros st st'. intros. inversion H1; subst.
  - (* If true *) apply (H st st'). assumption. split. assumption.
    apply bexp_eval_true. assumption.
  - (* If false *) apply (H0 st' st'). apply E_Skip. split. assumption.
...
Proof.
  apply hoare_if1.
  (* TODO: Need to copy these Theorems over ... what a pain. *)
  - eapply hoare_consequence_pre. apply hoare_asgn.
    unfold assn_sub, t_update, assert_implies, bassn.
Abort.
...
Proof.
  intros. apply hoare_consequence_pre with
    (fun st => P st \/ (Q st /\ ~ bassn b st)).
  - intros st st'. intros. remember (REPEAT c UNTIL b END) as repeat.
    induction H1; inversion Heqrepeat; subst.
    + (* Repeat End *) split.
      * { destruct H2.
        - apply (H st st'). assumption. assumption.
        - apply (H0 st st'). assumption. assumption. }
      * apply bexp_eval_true. assumption.
    + (* Repeat Loop *) apply IHceval2. assumption. right. split.
      * { destruct H2.
        - apply (H st st'). assumption. assumption.
        - apply (H0 st st'). assumption. assumption. }
      * apply bexp_eval_false. assumption.
  - unfold assert_implies. intros. left. assumption.
Qed.
...
Proof.
  intros. intros st st'. intros. inversion H; subst. apply H0.
Qed.
